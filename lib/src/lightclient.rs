...
impl LightClientConfig {
    ...
    pub async fn get_initial_state(&self, uri: &http::Uri, height: u64) -> Option<(u64, String, String)> {
        ...
        match grpcconnector::get_sapling_tree(uri, fetch_height as i32).await {
            Ok(tree_state) => {
...
impl LightClient {
    ...
    pub async fn set_wallet_initial_state(&self, height: u64) {
        ...
        let state = self.config.get_initial_state(&self.get_server_uri(), height).await;
    ...
    async fn new_wallet(config: &LightClientConfig, latest_block: u64, num_zaddres: u32, entropy: String) -> io::Result<Self>  {
        let mut l = LightClient {
    ...
    pub async fn new(config: &LightClientConfig, latest_block: u64, entropy: String) -> io::Result<Self> {
        ...
        Self::new_wallet(config, latest_block, 1, entropy).await
